...
class UTF16KIncrementalDecoder:
    ...
    def _on_error_unexpected_high_surrogate_plane_13_continuation(self) -> None:
        # XXX lots of scattered one-time error functions
        # Maybe we should have error functions only when strings are repeated
        # Otherwise we can have an _on_error(..., consume=True) or something
        # to determine whether a word should be consumed, of whether we should
        # back up and not decapitate the opening word of the next code unit
            ...
            if start_word < UNICODE_SURROGATE_HIGH_SUP:
                ...
                if start_word < UNICODE_SURROGATE_HIGH_PLANE_9_MIN:
                    ...
                    if start_word < UNICODE_SURROGATE_HIGH_PLANE_A_MIN:
                        ...
                        self._on_error_unexpected_high_surrogate_plane_13_continuation()
